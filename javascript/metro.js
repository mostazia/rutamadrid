// Linea 1

// Linea 1
const pinarDeChamartin = {
  lat: 40.48049736537236,
  long: -3.666733428122527,
  lines: [1, 4],
  name: "Pinar de Chamartín",
  id: "pinarDeChamartin",
};

const bambu = {
  lat: 40.47689815296868,
  long: -3.676726683828927,
  lines: [1],
  name: "Bambú",
  id: "bambu",
};

const chamartin = {
  lat: 40.47219295273449,
  long: -3.6836850067537408,
  lines: [1, 10],
  name: "Chamartín",
  id: "chamartin",
};

const plazaDeCastilla = {
  lat: 40.46685143294606,
  long: -3.6884795057233726,
  lines: [1, 9, 10],
  name: "Plaza de Castilla",
  id: "plazaDeCastilla",
};

const valdeacederas = {
  lat: 40.46458557527384,
  long: -3.6945571478315617,
  lines: [1],
  name: "Valdeacederas",
  id: "valdeacederas",
};

const tetuan = {
  lat: 40.46123992730214,
  long: -3.697895633525126,
  lines: [1],
  name: "Tetuán",
  id: "tetuan",
};

const estrecho = {
  lat: 40.454445341116546,
  long: -3.7038271810813574,
  lines: [1],
  name: "Estrecho",
  id: "estrecho",
};

const alvarado = {
  lat: 40.45025263871121,
  long: -3.7044568789842454,
  lines: [1],
  name: "Alvarado",
  id: "alvarado",
};

const cuatroCaminos = {
  lat: 40.44698813926112,
  long: -3.7039846056202093,
  lines: [1, 4, 6],
  name: "Cuatro Caminos",
  id: "cuatroCaminos",
};

const riosRosas = {
  lat: 40.44169434326654,
  long: -3.700901088154907,
  lines: [1],
  name: "Ríos Rosas",
  id: "riosRosas",
};

const iglesia = {
  lat: 40.43538508919413,
  long: -3.697397411209908,
  lines: [1],
  name: "Iglesia",
  id: "iglesia",
};

const bilbao = {
  lat: 40.429509219211845,
  long: -3.70226708653278,
  lines: [1, 4],
  name: "Bilbao",
  id: "bilbao",
};

const tribunal = {
  lat: 40.426164402933864,
  long: -3.7012198720570364,
  lines: [1, 10],
  name: "Tribunal",
  id: "tribunal",
};

const granVia = {
  lat: 40.419883424160226,
  long: -3.701625430255096,
  lines: [1, 5],
  name: "Gran Vía",
  id: "granVia",
};

const sol = {
  lat: 40.41718266614042,
  long: -3.703206505912717,
  lines: [1, 2, 3],
  name: "Sol",
  id: "sol",
};

const tirsoDeMolina = {
  lat: 40.41236299045606,
  long: -3.703887111258177,
  lines: [1],
  name: "Tirso de Molina",
  id: "tirsoDeMolina",
};

const antonMartin = {
  lat: 40.41247108851637,
  long: -3.699635803505414,
  lines: [1],
  name: "Antón Martín",
  id: "antonMartin",
};

const estacionDelArte = {
  lat: 40.40903295135812,
  long: -3.691454322130659,
  lines: [1],
  name: "Estación del Arte",
  id: "estacionDelArte",
};

const atocha = {
  lat: 40.4051657580651,
  long: -3.6880658414714054,
  lines: [1],
  name: "Atocha",
  id: "atocha",
};

const menendezPelayo = {
  lat: 40.40451680910258,
  long: -3.6807882300400885,
  lines: [1],
  name: "Menéndez Pelayo",
  id: "menendezPelayo",
};

const pacifico = {
  lat: 40.40173458990905,
  long: -3.67525304188166,
  lines: [1, 6],
  name: "Pacífico",
  id: "pacifico",
};

const puenteDeVallecas = {
  lat: 40.398576631774134,
  long: -3.669736121595171,
  lines: [1],
  name: "Puente de Vallecas",
  id: "puenteDeVallecas",
};

const nuevaNumancia = {
  lat: 40.3957941670959,
  long: -3.6658998526406434,
  lines: [1],
  name: "Nueva Numancia",
  id: "nuevaNumancia",
};

const portazgo = {
  lat: 40.39292810824377,
  long: -3.6591407120943438,
  lines: [1],
  name: "Portazgo",
  id: "portazgo",
};

const buenosAires = {
  lat: 40.391564600515146,
  long: -3.6554140507456077,
  lines: [1],
  name: "Buenos Aires",
  id: "buenosAires",
};

const altoDeArenal = {
  lat: 40.38989090160141,
  long: -3.646215055618432,
  lines: [1],
  name: "Alto de Arenal",
  id: "altoDeArenal",
};

const miguelHernandez = {
  lat: 40.38740463377381,
  long: -3.6392842788848676,
  lines: [1],
  name: "Miguel Hernandez",
  id: "miguelHernandez",
};

const sierraDeGuadalupe = {
  lat: 40.382503366543794,
  long: -3.6252283466812516,
  lines: [1],
  name: "Sierra de Guadalupe",
  id: "sierraDeGuadalupe",
};

const villaDeVallecas = {
  lat: 40.379735277471724,
  long: -3.621726154925654,
  lines: [1],
  name: "Villa de Vallecas",
  id: "villaDeVallecas",
};

const congosto = {
  lat: 40.372214479829836,
  long: -3.618992294160439,
  lines: [1],
  name: "Congosto",
  id: "congosto",
};

const laGavia = {
  lat: 40.36929271283607,
  long: -3.612917048288414,
  lines: [1],
  name: "La Gavia",
  id: "laGavia",
};

const lasSuertes = {
  lat: 40.36317394875008,
  long: -3.6001400465089017,
  lines: [1],
  name: "Las Suertes",
  id: "lasSuertes",
};

const valdecarros = {
  lat: 40.360034792782095,
  long: -3.5934762611520554,
  lines: [1],
  name: "Valdecarros",
  id: "valdecarros",
};

// Linea 2

const lasRosas = {
  lat: 40.424226611036026,
  long: -3.600801478897012,
  lines: [2],
  name: "Las Rosas",
  id: "lasRosas",
};

const avenidaDeGuadalajara = {
  id: "avenidaDeGuadalajara",
  lat: 40.42237354712535,
  long: -3.6131624443051567,
  lines: [2],
  name: "Avenida de Guadalajara",
};

const alsacia = {
  id: "alsacia",
  lat: 40.418783090522325,
  long: -3.624610600138953,
  lines: [2],
  name: "Alsacia",
};

const laAlmudena = {
  id: "laAlmudena",
  lat: 40.42356067211956,
  long: -3.640508703686634,
  lines: [2],
  name: "La Almudena",
};

const laElipa = {
  id: "laElipa",
  lat: 40.42648496365145,
  long: -3.651386353148477,
  lines: [2],
  name: "La Elipa",
};

const ventas = {
  id: "ventas",
  lat: 40.43105934292208,
  long: -3.6636332177135538,
  lines: [2, 5],
  name: "Ventas",
};

const manuelBecerra = {
  id: "manuelBecerra",
  lat: 40.42775887266891,
  long: -3.672266876766128,
  lines: [2, 6],
  name: "Manuel Becerra",
};

const goya = {
  id: "goya",
  lat: 40.42466091547799,
  long: -3.674891205041604,
  lines: [2, 4],
  name: "Goya",
};

const principeDeVergara = {
  id: "principeDeVergara",
  lat: 40.42377567589764,
  long: -3.678244979170257,
  lines: [2, 9],
  name: "Príncipe de Vergara",
};

const retiro = {
  id: "retiro",
  lat: 40.42126002859873,
  long: -3.684510619231018,
  lines: [2],
  name: "Retiro",
};

const bancoDeEspana = {
  id: "bancoDeEspana",
  lat: 40.41965911309961,
  long: -3.69347992633552,
  lines: [2],
  name: "Banco de España",
};

const sevilla = {
  id: "sevilla",
  lat: 40.41786212181181,
  long: -3.698844344195753,
  lines: [2],
  name: "Sevilla",
};

const opera = {
  id: "opera",
  lat: 40.41835221503397,
  long: -3.708586127511464,
  lines: [2, 5, 13],
  name: "Ópera",
};

const santoDomingo = {
  id: "santoDomingo",
  lat: 40.42201024391963,
  long: -3.7063004936253123,
  lines: [2],
  name: "Santo Domingo",
};

const sanBernardo = {
  id: "sanBernardo",
  lat: 40.42979581024937,
  long: -3.7052670785846282,
  lines: [2, 4],
  name: "San Bernardo",
};

const quevedo = {
  id: "quevedo",
  lat: 40.43320352447881,
  long: -3.7056152797588413,
  lines: [2],
  name: "Quevedo",
};

const canal = {
  id: "canal",
  lat: 40.43827691239186,
  long: -3.7049188773577444,
  lines: [2, 7],
  name: "Canal",
};

// Linea 3

const villaverdeAlto = {
  id: "villaverdeAlto",
  lat: 40.34157696493295,
  long: -3.7118549354131285,
  lines: [3],
  name: "Villaverde Alto",
};

const sanCristobal = {
  id: "sanCristobal",
  lat: 40.34166643542084,
  long: -3.6912072676024468,
  lines: [3],
  name: "San Cristobal",
};

const villaverdeBajoCruce = {
  id: "villaverdeBajoCruce",
  lat: 40.351042040301834,
  long: -3.691778243371878,
  lines: [3],
  name: "Villaverde Bajo-Cruce",
};

const ciudadDeLosAngeles = {
  id: "ciudadDeLosAngeles",
  lat: 40.3597995620305,
  long: -3.6948929952957545,
  lines: [3],
  name: "Ciudad de Los Angeles",
};

const sanFerminOrcasur = {
  id: "sanFerminOrcasur",
  lat: 40.36980560138429,
  long: -3.6948872160351183,
  lines: [3],
  name: "San Fermín-Orcasur",
};

const hospital12deOctubre = {
  id: "hospital12deOctubre",
  lat: 40.37537318189435,
  long: -3.6968264075946204,
  lines: [3],
  name: "Hospital 12 de Octubre",
};

const almendrales = {
  id: "almendrales",
  lat: 40.383887315106875,
  long: -3.6989814500418494,
  lines: [3],
  name: "Almendrales",
};

const legazpi = {
  id: "legazpi",
  lat: 40.391169527433696,
  long: -3.6959028601620707,
  lines: [3, 6],
  name: "Legazpi",
};

const delicias = {
  id: "delicias",
  lat: 40.39977949950838,
  long: -3.694774043808453,
  lines: [3],
  name: "Delicias",
};

const palosDeLaFrontera = {
  id: "palosDeLaFrontera",
  lat: 40.40295748631128,
  long: -3.6950305929471727,
  lines: [3],
  name: "Palos de la Frontera",
};

const embajadores = {
  id: "embajadores",
  lat: 40.40468968643914,
  long: -3.703223062802927,
  lines: [3, 5],
  name: "Embajadores / Acacias",
};

const lavapies = {
  id: "lavapies",
  lat: 40.4084465813102,
  long: -3.701215529310202,
  lines: [3],
  name: "Lavapiés",
};

const callao = {
  id: "callao",
  lat: 40.420274936011005,
  long: -3.7062224525949627,
  lines: [3, 5],
  name: "Callao",
};

const plazaDeEspana = {
  id: "plazaDeEspana",
  lat: 40.42364729870026,
  long: -3.711322963186548,
  lines: [2, 3, 10],
  name: "Noviciado / Plaza de España",
};

const venturaRodriguez = {
  id: "venturaRodriguez",
  lat: 40.42704323961471,
  long: -3.714270964781492,
  lines: [3],
  name: "Ventura Rodriguez",
};

const arguelles = {
  id: "arguelles",
  lat: 40.43072280543736,
  long: -3.7157988549626175,
  lines: [3, 4, 6],
  name: "Argüelles",
};

const moncloa = {
  id: "moncloa",
  lat: 40.434503876540155,
  long: -3.719393014988317,
  lines: [3, 6],
  name: "Moncloa",
};

// Linea 4

const alonsoMartinez = {
  id: "alonsoMartinez",
  lat: 40.427898410000076,
  long: -3.6954946512626874,
  lines: [4, 5, 10],
  name: "Alonso Martínez",
};

const colon = {
  id: "colon",
  lat: 40.42543192087801,
  long: -3.6914498800743325,
  lines: [4],
  name: "Colón",
};

const serrano = {
  id: "serrano",
  lat: 40.4253747495098,
  long: -3.6871368879929665,
  lines: [4],
  name: "Serrano",
};

const velazquez = {
  id: "velazquez",
  lat: 40.425186900444324,
  long: -3.683360337666426,
  lines: [4],
  name: "Velázquez",
};

const lista = {
  id: "lista",
  lat: 40.42914795439827,
  long: -3.6757535929337624,
  lines: [4],
  name: "Lista",
};

const diegoDeLeon = {
  id: "diegoDeLeon",
  lat: 40.43462279287774,
  long: -3.6756630979616842,
  lines: [4, 5, 6],
  name: "Diego de León",
};

const avenidaDeAmerica = {
  id: "avenidaDeAmerica",
  lat: 40.44461714984074,
  long: -3.6751087479833727,
  lines: [4, 7],
  name: "Avenida de América",
};

const prosperidad = {
  id: "prosperidad",
  lat: 40.4441206249458,
  long: -3.675720061061254,
  lines: [4],
  name: "Prosperidad",
};

const alfonsoXIII = {
  id: "alfonsoXIII",
  lat: 40.44822067290521,
  long: -3.668327390075695,
  lines: [4],
  name: "Alfonso XIII",
};

const avenidaDeLaPaz = {
  id: "avenidaDeLaPaz",
  lat: 40.4533804796333,
  long: -3.6613965619764652,
  lines: [4],
  name: "Avenida de la Paz",
};

const arturoSoria = {
  id: "arturoSoria",
  lat: 40.45592757993377,
  long: -3.6578131307210393,
  lines: [4],
  name: "Arturo Soria",
};

const esperanza = {
  id: "esperanza",
  lat: 40.459486827566316,
  long: -3.6470199217278525,
  lines: [4],
  name: "Esperanza",
};

const canillas = {
  id: "canillas",
  lat: 40.46443354204033,
  long: -3.6358833898885248,
  lines: [4],
  name: "Canillas",
};

const marDeCristal = {
  id: "marDeCristal",
  lat: 40.4694778361166,
  long: -3.6376214612128437,
  lines: [4, 8],
  name: "Mar de Cristal",
};

const sanLorenzo = {
  id: "sanLorenzo",
  lat: 40.47432587822357,
  long: -3.6405826199368,
  lines: [4],
  name: "San Lorenzo",
};

const parqueDeSantaMaria = {
  id: "parqueDeSantaMaria",
  lat: 40.477247593121064,
  long: -3.645882665054671,
  lines: [4],
  name: "Parque de Santa María",
};

const hortaleza = {
  id: "hortaleza",
  lat: 40.47516910507403,
  long: -3.653356099221605,
  lines: [4],
  name: "Hortaleza",
};

const manoteras = {
  id: "manoteras",
  lat: 40.47655761229688,
  long: -3.663116140261856,
  lines: [4],
  name: "Manoteras",
};

// Linea 5

const alamedaDeOsuna = {
  id: "alamedaDeOsuna",
  lat: 40.45780158612176,
  long: -3.588017457032416,
  lines: [5],
  name: "Alameda de Osuna",
};

const elCapricho = {
  id: "elCapricho",
  lat: 40.453377481917464,
  long: -3.595605525872296,
  lines: [5],
  name: "El Capricho",
};

const canillejas = {
  id: "canillejas",
  lat: 40.44930929848327,
  long: -3.6093281050168033,
  lines: [5],
  name: "Canillejas",
};

const torreArias = {
  id: "torreArias",
  lat: 40.44363919852365,
  long: -3.6166671694238586,
  lines: [5],
  name: "Torre Arias",
};

const suanzes = {
  id: "suanzes",
  lat: 40.440894575096884,
  long: -3.6276098659606713,
  lines: [5],
  name: "Suanzes",
};

const ciudadLineal = {
  id: "ciudadLineal",
  lat: 40.437840377113396,
  long: -3.638517206613873,
  lines: [5],
  name: "Ciudad Lineal",
};

const puebloNuevo = {
  id: "puebloNuevo",
  lat: 40.435593367522465,
  long: -3.643608477504308,
  lines: [5, 7],
  name: "Pueblo Nuevo",
};

const quintana = {
  id: "quintana",
  lat: 40.43357619158819,
  long: -3.646692463517938,
  lines: [5],
  name: "Quintana",
};

const elCarmen = {
  id: "elCarmen",
  lat: 40.43186476607949,
  long: -3.6566229852588914,
  lines: [5],
  name: "El Carmen",
};

const nunezDeBalboa = {
  id: "nunezDeBalboa",
  lat: 40.43278769606949,
  long: -3.68398149676216,
  lines: [5],
  name: "Nuñez de Balboa",
};

const rubenDario = {
  id: "rubenDario",
  lat: 40.43318675129849,
  long: -3.6896472135356944,
  lines: [5],
  name: "Rubén Darío",
};

const chueca = {
  id: "chueca",
  lat: 40.42264228661484,
  long: -3.6981355257855095,
  lines: [5],
  name: "Chueca",
};

const laLatina = {
  id: "laLatina",
  lat: 40.41141549329365,
  long: -3.708241149415657,
  lines: [5],
  name: "La Latina",
};

const puertaDeToledo = {
  id: "puertaDeToledo",
  lat: 40.40680069908103,
  long: -3.7103857192123533,
  lines: [5],
  name: "Puerta de Toledo",
};

const acacias = {
  id: "acacias",
  lat: 40.40372963012126,
  long: -3.707143731303211,
  lines: [3, 5],
  name: "Embajadores / Acacias",
};

const piramides = {
  id: "piramides",
  lat: 40.40251498859207,
  long: -3.7122790401821546,
  lines: [5],
  name: "Pirámides",
};

const marquesDeVadillo = {
  id: "marquesDeVadillo",
  lat: 40.397497554264866,
  long: -3.7157475669085125,
  lines: [5],
  name: "Marqués de Vadillo",
};

const urgel = {
  id: "urgel",
  lat: 40.39302359608587,
  long: -3.7249159089096966,
  lines: [5],
  name: "Urgel",
};

const oporto = {
  id: "oporto",
  lat: 40.38847032241529,
  long: -3.7310238143364813,
  lines: [5, 6],
  name: "Oporto",
};

const vistaAlegre = {
  id: "vistaAlegre",
  lat: 40.388648113280524,
  long: -3.7404774512077097,
  lines: [5],
  name: "Vista Alegre",
};

const carabanchel = {
  id: "carabanchel",
  lat: 40.387818418554076,
  long: -3.745353401167654,
  lines: [5],
  name: "Carabanchel",
};

const eugeniaDeMontijo = {
  id: "eugeniaDeMontijo",
  lat: 40.384687216687595,
  long: -3.7512797552461685,
  lines: [5],
  name: "Eugenia de Montijo",
};

const aluche = {
  id: "aluche",
  lat: 40.38567499322186,
  long: -3.761239142321913,
  lines: [5],
  name: "Aluche",
};

const empalme = {
  id: "empalme",
  lat: 40.390840828674534,
  long: -3.7665041308630385,
  lines: [5],
  name: "Empalme",
};

const campamento = {
  id: "campamento",
  lat: 40.39458408322918,
  long: -3.768734618563986,
  lines: [5],
  name: "Campamento",
};

const casaDeCampo = {
  id: "casaDeCampo",
  lat: 40.40318586646646,
  long: -3.7617967643248202,
  lines: [5, 10],
  name: "Casa de Campo",
};

// Linea 6

const laguna = {
  id: "laguna",
  lat: 40.39920804089135,
  long: -3.7438293666073954,
  lines: [6],
  name: "Laguna",
};

const carpetana = {
  id: "carpetana",
  lat: 40.39277002430141,
  long: -3.741659530507246,
  lines: [6],
  name: "Carpetana",
};

const opanel = {
  id: "opanel",
  lat: 40.386152064852965,
  long: -3.7238134745409734,
  lines: [6],
  name: "Opañel",
};

const plazaEliptica = {
  id: "plazaEliptica",
  lat: 40.38446530816853,
  long: -3.71928947497992,
  lines: [6, 11],
  name: "Plaza Elíptica",
};

const usera = {
  id: "usera",
  lat: 40.387082841671855,
  long: -3.7068449326330994,
  lines: [6],
  name: "Usera",
};

const arganzuelaPlanetario = {
  id: "arganzuelaPlanetario",
  lat: 40.39331263698368,
  long: -3.68911764546199,
  lines: [6],
  name: "Arganzuela-Planetario",
};

const mendezAlvaro = {
  id: "mendezAlvaro",
  lat: 40.395616809624904,
  long: -3.6790958983674438,
  lines: [6],
  name: "Méndez Álvaro",
};

const condeDeCasal = {
  id: "condeDeCasal",
  lat: 40.40710049399163,
  long: -3.671311711136777,
  lines: [6],
  name: "Conde de Casal",
};

const sainzDeBaranda = {
  id: "sainzDeBaranda",
  lat: 40.41477918986336,
  long: -3.6708705021436265,
  lines: [6, 9],
  name: "Sainz de Baranda",
};

const odonnell = {
  id: "odonnell",
  lat: 40.42239702992558,
  long: -3.6697832370923855,
  lines: [6],
  name: "O'Donenll",
};

const republicaArgentina = {
  id: "republicaArgentina",
  lat: 40.44371551203646,
  long: -3.6851297050848206,
  lines: [6],
  name: "República Argentina",
};

const nuevosMinisterios = {
  id: "nuevosMinisterios",
  lat: 40.44659355845692,
  long: -3.6938120676626984,
  lines: [6, 8, 10],
  name: "Nuevos Ministerios",
};

const guzmanElBueno = {
  id: "guzmanElBueno",
  lat: 40.44653360040284,
  long: -3.712327088050442,
  lines: [6, 7],
  name: "Guzmán el Bueno",
};

const vicenteAleixandre = {
  id: "vicenteAleixandre",
  lat: 40.44680940696806,
  long: -3.719843398316318,
  lines: [6],
  name: "Vicente Aleixandre",
};

const ciudadUniversitaria = {
  id: "ciudadUniversitaria",
  lat: 40.443583598656645,
  long: -3.727501525713917,
  lines: [6],
  name: "Ciudad Universitaria",
};

const principePio = {
  id: "principioPio",
  lat: 40.42098701853868,
  long: -3.7199886590318756,
  lines: [6, 10, 13],
  name: "Príncipe Pío",
};

const puertaDelAngel = {
  id: "puertaDelAngel",
  lat: 40.41421422647763,
  long: -3.728471344413345,
  lines: [6],
  name: "Puerta del Ángel",
};

const altoDeExtremadura = {
  id: "altoDeExtremadura",
  lat: 40.409819605263856,
  long: -3.7403603060892574,
  lines: [6],
  name: "Alto de Extremadura",
};

const lucero = {
  id: "lucero",
  lat: 40.404739859191444,
  long: -3.746161259459823,
  lines: [6],
  name: "Lucero",
};

// Linea 7

const hospitalDeHenares = {
  id: "hospitalDeHenares",
  lat: 40.4179059813681,
  long: -3.5329948676912686,
  lines: [7],
  name: "Hospital de Henares",
};

const henares = {
  id: "henares",
  lat: 40.41776760124066,
  long: -3.5274695952639523,
  lines: [7],
  name: "Henares",
};

const jarama = {
  id: "jarama",
  lat: 40.42291532846304,
  long: -3.525650728860471,
  lines: [7],
  name: "Jarama",
};

const sanFernando = {
  id: "sanFernando",
  lat: 40.42442226462559,
  long: -3.535933941875459,
  lines: [7],
  name: "San Fernando",
};

const laRambla = {
  id: "laRambla",
  lat: 40.4251716305573,
  long: -3.548245940087711,
  lines: [7],
  name: "La Rambla",
};

const cosladaCentral = {
  id: "cosladaCentral",
  lat: 40.42320514297449,
  long: -3.560610514798939,
  lines: [7],
  name: "Coslada Central",
};

const barrioDelPuerto = {
  id: "barrioDelPuerto",
  lat: 40.422474496591875,
  long: -3.569554666094362,
  lines: [7],
  name: "Barrio del Puerto",
};

const estadioMetropolitano = {
  id: "estadioMetropolitano",
  lat: 40.433396072386955,
  long: -3.601223184846861,
  lines: [7],
  name: "Estadio Metropolitano",
};

const lasMusas = {
  id: "lasMusas",
  lat: 40.43265290118605,
  long: -3.607938963235132,
  lines: [7],
  name: "Las Musas",
};

const sanBlas = {
  id: "sanBlas",
  lat: 40.42796078019834,
  long: -3.616378540077953,
  lines: [7],
  name: "San Blas",
};

const simancas = {
  id: "simancas",
  lat: 40.42785817916719,
  long: -3.6263125995028775,
  lines: [7],
  name: "Simancas",
};

const garciaNoblejas = {
  id: "garciaNoblejas",
  lat: 40.42908203238928,
  long: -3.6312722968282984,
  lines: [7],
  name: "García Noblejas",
};

const ascao = {
  id: "ascao",
  lat: 40.43017977269128,
  long: -3.6414450353740806,
  lines: [7],
  name: "Acscao",
};

const barrioDeLaConcepcion = {
  id: "barrioDeLaConcepcion",
  lat: 40.43897425478377,
  long: -3.6513405901275675,
  lines: [7],
  name: "Barrio de la Concepción",
};

const parqueDeLasAvenidas = {
  id: "parqueDeLasAvenidas",
  lat: 40.43945391011527,
  long: -3.6637770825521496,
  lines: [7],
  name: "Parque de las Avenidas",
};

const cartagena = {
  id: "cartagena",
  lat: 40.43927598873854,
  long: -3.6724421320116383,
  lines: [7],
  name: "Cartagena",
};

const gregorioMaranon = {
  id: "gregorioMaranon",
  lat: 40.4380927785094,
  long: -3.6918975206064792,
  lines: [7, 10],
  name: "Gregorio Marañón",
};

const alonsoCano = {
  id: "alonsoCano",
  lat: 40.438229105715884,
  long: -3.7048195430693203,
  lines: [7],
  name: "Alonso Cano",
};

const islasFilipinas = {
  id: "islasFilipinas",
  lat: 40.439134700913534,
  long: -3.7143127515618457,
  lines: [7],
  name: "Islas Filipinas",
};

const francosRodriguez = {
  id: "francosRodriguez",
  lat: 40.45651392221677,
  long: -3.7132508428100275,
  lines: [7],
  name: "Francos Rodríguez",
};

const valdezarza = {
  id: "valdezarza",
  lat: 40.46467135659308,
  long: -3.7167436271532397,
  lines: [7],
  name: "Valdezarza",
};

const antonioMachado = {
  id: "antonioMachado",
  lat: 40.469959547595835,
  long: -3.7186278293684945,
  lines: [7],
  name: "Antonio Machado",
};

const penagrande = {
  id: "penagrande",
  lat: 40.475898830653804,
  long: -3.71663121912141,
  lines: [7],
  name: "Peñagrande",
};

const avenidaDeLaIlustracion = {
  id: "avenidaDeLaIlustracion",
  lat: 40.480105941909066,
  long: -3.7196282068200293,
  lines: [7],
  name: "Avanida de la Ilustración",
};

const lacoma = {
  id: "lacoma",
  lat: 40.4848577912663,
  long: -3.72354532818288,
  lines: [7],
  name: "Lacoma",
};

const arroyofresno = {
  id: "arroyofresno",
  lat: 40.49086563242766,
  long: -3.7266744552073057,
  lines: [7],
  name: "Arroyofresno",
};

const pitis = {
  id: "pitis",
  lat: 40.49540860116347,
  long: -3.7266306911750293,
  lines: [7],
  name: "Pitis",
};

// Linea 8

const colombia = {
  id: "colombia",
  lat: 40.45748814267888,
  long: -3.676657192661387,
  lines: [8, 9],
  name: "Colombia",
};

const pinarDelRey = {
  id: "pinarDelRey",
  lat: 40.468039974220545,
  long: -3.6476784666030384,
  lines: [8],
  name: "Pinar del Rey",
};

const feriaDeMadrid = {
  id: "feriaDeMadrid",
  lat: 40.46401335967577,
  long: -3.6169790119212184,
  lines: [8],
  name: "Feria de Madrid",
};

const aeropuertoT1T2T3 = {
  id: "aeropuertoT1T2T3",
  lat: 40.46814388193424,
  long: -3.5727782620198623,
  lines: [8],
  name: "Aeropuerto T1-T2-T3",
};

const barajas = {
  id: "barajas",
  lat: 40.47579755878603,
  long: -3.5835126876175694,
  lines: [8],
  name: "Barajas",
};

const aeropuertoT4 = {
  id: "aeropuertoT4",
  lat: 40.49128303925498,
  long: -3.5945838019230965,
  lines: [8],
  name: "Aeropuerto T4",
};

const pacoDeLucia = {
  id: "pacoDeLucia",
  lat: 40.499825374941366,
  long: -3.7080929518006926,
  lines: [9],
  name: "Paco de Lucía",
};

const mirasierra = {
  id: "mirasierra",
  lat: 40.4909648845967,
  long: -3.7173841237848766,
  lines: [9],
  name: "Mirasierra",
};

const herreraOria = {
  id: "herreraOria",
  lat: 40.48463291504563,
  long: -3.708758139583486,
  lines: [9],
  name: "Herrera Oria",
};

const barrioDelPilar = {
  id: "barrioDelPilar",
  lat: 40.47626008981339,
  long: -3.7047884701993956,
  lines: [9],
  name: "Barrio del Pilar",
};

const ventilla = {
  id: "ventilla",
  lat: 40.46961657917464,
  long: -3.6950037718907587,
  lines: [9],
  name: "Ventilla",
};

const duqueDePastrana = {
  id: "duqueDePastrana",
  lat: 40.46760870577372,
  long: -3.6818716764738153,
  lines: [9],
  name: "Duque de Pastrana",
};

const pioXII = {
  id: "pioXII",
  lat: 40.46251409419988,
  long: -3.6766669521064124,
  lines: [9],
  name: "Pío XIII",
};

const conchaEspina = {
  id: "conchaEspina",
  lat: 40.45048459991799,
  long: -3.6790580178767973,
  lines: [9],
  name: "Concha Espina",
};

const cruzDelRayo = {
  id: "cruzDelRayo",
  lat: 40.444319667901425,
  long: -3.678915267695397,
  lines: [9],
  name: "Cruz del Rayo",
};

const ibiza = {
  id: "ibiza",
  lat: 40.4182955742016,
  long: -3.6776690581866416,
  lines: [9],
  name: "Ibiza",
};

const estrella = {
  id: "estrella",
  lat: 40.411502719082364,
  long: -3.6613241605554996,
  lines: [9],
  name: "Estrella",
};

const vinateros = {
  id: "vinateros",
  lat: 40.41021198977017,
  long: -3.6512662826413433,
  lines: [9],
  name: "Vinateros",
};

const artilleros = {
  id: "artilleros",
  lat: 40.405239862392015,
  long: -3.642472139151465,
  lines: [9],
  name: "Artilleros",
};

const pavones = {
  id: "pavones",
  lat: 40.40060134708578,
  long: -3.6358732902136675,
  lines: [9],
  name: "Pavones",
};

const valdebernardo = {
  id: "valdebernardo",
  lat: 40.399859810310964,
  long: -3.6218737731777684,
  lines: [9],
  name: "Valdebernardo",
};

const vicalvaro = {
  id: "vicalvaro",
  lat: 40.40423874498485,
  long: -3.609855966100855,
  lines: [9],
  name: "Vicálvaro",
};

const sanCipriano = {
  id: "sanCipriano",
  lat: 40.40408740481849,
  long: -3.602909716878376,
  lines: [9],
  name: "San Cipriano",
};

const puertaDeArganda = {
  id: "puertaDeArganda",
  lat: 40.401597355607805,
  long: -3.5950436248212094,
  lines: [9],
  name: "Puerta de Arganda",
};

const rivasUrbanizaciones = {
  id: "rivasUrbanizaciones",
  lat: 40.367351769669064,
  long: -3.546605405780413,
  lines: [9],
  name: "Rivas Urbanizaciones",
};

const rivasFutura = {
  id: "rivasFutura",
  lat: 40.341713433967236,
  long: -3.52690293951884,
  lines: [9],
  name: "Rivas Futura",
};

const rivasVaciamadrid = {
  id: "rivasVaciamadrid",
  lat: 40.3281751767282,
  long: -3.5184901363895524,
  lines: [9],
  name: "Rivas Vaciamadrid",
};

const laPoveda = {
  id: "laPoveda",
  lat: 40.31891059381465,
  long: -3.4801651450895275,
  lines: [9],
  name: "La Poveda",
};

const argandaDelRey = {
  id: "argandaDelRey",
  lat: 40.303650415091234,
  long: -3.449403178096834,
  lines: [9],
  name: "Arganda del Rey",
};

// Linea 10
const hospitalInfantaSofia = {
  id: "hospitalInfantaSofia",
  lat: 40.560110329013575,
  long: -3.610162732753923,
  lines: [10],
  name: "Hospital Infanta Sofía",
};

const reyesCatolicos = {
  id: "reyesCatolicos",
  lat: 40.5503527288015,
  long: -3.624223004031726,
  lines: [10],
  name: "Reyes Católicos",
};

const baunatal = {
  id: "baunatal",
  lat: 40.55428797584786,
  long: -3.6359568927658854,
  lines: [10],
  name: "Baunatal",
};

const manuelDeFalla = {
  id: "manuelDeFalla",
  lat: 40.550665860343024,
  long: -3.647756936397305,
  lines: [10],
  name: "Manuel de Falla",
};

const marquesDeLaValdavia = {
  id: "marquesDeLaValdavia",
  lat: 40.54107074526154,
  long: -3.6386590769730227,
  lines: [10],
  name: "Marqués de la Valdavia",
};

const laMoraleja = {
  id: "laMoraleja",
  lat: 40.53190558802484,
  long: -3.6366727996800705,
  lines: [10],
  name: "La Moraleja",
};

const laGranja = {
  id: "laGranja",
  lat: 40.52763196999366,
  long: -3.657700339172358,
  lines: [10],
  name: "La Granja",
};

const rondaDeLaComunicacion = {
  id: "rondaDeLaComunicacion",
  lat: 40.51574788523366,
  long: -3.6645868824021877,
  lines: [10],
  name: "Ronda de La Comunicación",
};

const lasTablas = {
  id: "lasTablas",
  lat: 40.508234293515294,
  long: -3.670168606991995,
  lines: [10],
  name: "Las Tablas",
};

const montecarmelo = {
  id: "montecarmelo",
  lat: 40.505276389563086,
  long: -3.6967240839489315,
  lines: [10],
  name: "Montecarmelo",
};

const tresOlivos = {
  id: "tresOlivos",
  lat: 40.50048100101604,
  long: -3.6955884087194493,
  lines: [10],
  name: "Tres Olivos",
};

const fuencarral = {
  id: "fuencarral",
  lat: 40.49507200148446,
  long: -3.6941834710040604,
  lines: [10],
  name: "Fuencarral",
};

const begona = {
  id: "begona",
  lat: 40.48042983236294,
  long: -3.686525042430225,
  lines: [10],
  name: "Begoña",
};

const cuzco = {
  id: "cuzco",
  lat: 40.4583410809721,
  long: -3.6909101704938823,
  lines: [10],
  name: "Cuzco",
};

const santiagoBernabeu = {
  id: "santiagoBernabeu",
  lat: 40.45209998745305,
  long: -3.690094161806205,
  lines: [10],
  name: "Santiago Bernabeú",
};

const lago = {
  id: "lago",
  lat: 40.41668380794701,
  long: -3.7349151046922238,
  lines: [10],
  name: "Lago",
};

const batan = {
  id: "batan",
  lat: 40.407639387815244,
  long: -3.753571142027108,
  lines: [10],
  name: "Batán",
};

const coloniaJardin = {
  id: "coloniaJardin",
  lat: 40.397212808344996,
  long: -3.775044331845643,
  lines: [10],
  name: "Colonia Jardín",
};

const aviacionEspanola = {
  id: "aviacionEspanola",
  lat: 40.38378312207911,
  long: -3.785194841015032,
  lines: [10],
  name: "Aviación Española",
};

const cuatroVientos = {
  id: "cuatroVientos",
  lat: 40.37787473167457,
  long: -3.7925022099532435,
  lines: [10],
  name: "Cuatro Vientos",
};

const joaquinVilumbrales = {
  id: "joaquinVilumbrales",
  lat: 40.34974034029154,
  long: -3.8095765570155797,
  lines: [10],
  name: "Joaquin Vilumbrales",
};

const puertaDelSur = {
  id: "puertaDelSur",
  lat: 40.34536863369864,
  long: -3.812993313042291,
  lines: [10, 12],
  name: "Puerta del Sur",
};

// Línea 11
const abrantes = {
  id: "abrantes",
  lat: 40.38101682028277,
  long: -3.7266040721309106,
  lines: [11],
  name: "Abrantes",
};

const panBendito = {
  id: "panBendito",
  lat: 40.375968527054745,
  long: -3.735630092306251,
  lines: [11],
  name: "Pan Bendito",
};

const sanFrancisco = {
  id: "sanFrancisco",
  lat: 40.37381688313462,
  long: -3.740836953502392,
  lines: [11],
  name: "San Francisco",
};

const carabanchelAlto = {
  id: "carabanchelAlto",
  lat: 40.37189641963218,
  long: -3.751469241468814,
  lines: [11],
  name: "Carabanchel Alto",
};

const laPeseta = {
  id: "laPeseta",
  lat: 40.363185835142424,
  long: -3.7585748055067283,
  lines: [11],
  name: "La Peseta",
};

const laFortuna = {
  id: "laFortuna",
  lat: 40.35813368947538,
  long: -3.776571223036998,
  lines: [11],
  name: "La Fortuna",
};

// Línea 12
const parqueDeLisboa = {
  id: "parqueDeLisboa",
  lat: 40.349656673411616,
  long: -3.822716500031455,
  lines: [12],
  name: "Parque de Lisboa",
};

const alcorconCentral = {
  id: "alcorconCentral",
  lat: 40.35026944159127,
  long: -3.832700095755241,
  lines: [12],
  name: "Alcorcón Central",
};

const parqueOeste = {
  id: "parqueOeste",
  lat: 40.345979947183295,
  long: -3.849875454411215,
  lines: [12],
  name: "Parque Oeste",
};

const universidadReyJuanCarlos = {
  id: "universidadReyJuanCarlos",
  lat: 40.334674224484,
  long: -3.872801530092762,
  lines: [12],
  name: "Universidad Rey Juan Carlos",
};

const mostolesCentral = {
  id: "mostolesCentral",
  lat: 40.32861922039742,
  long: -3.8648011530781274,
  lines: [12],
  name: "Móstoles Central",
};

const pradillo = {
  id: "pradillo",
  lat: 40.32164434985196,
  long: -3.8651073876242226,
  lines: [12],
  name: "Pradillo",
};

const hospitalDeMostoles = {
  id: "hospitalDeMostoles",
  lat: 40.31634705240296,
  long: -3.87584473565684,
  lines: [12],
  name: "Hospital de Móstoles",
};

const manuelaMalasana = {
  id: "manuelaMalasana",
  lat: 40.30900257875122,
  long: -3.865452424826645,
  lines: [12],
  name: "Manuela Malasaña",
};

const loranca = {
  id: "loranca",
  lat: 40.29652247381484,
  long: -3.838637764178445,
  lines: [12],
  name: "Loranca",
};

const hospitalDeFuenlabrada = {
  id: "hospitalDeFuenlabrada",
  lat: 40.28583592051701,
  long: -3.8193067096176763,
  lines: [12],
  name: "Hospital de Fuenlabrada",
};

const parqueEuropa = {
  id: "parqueEuropa",
  lat: 40.285187511819245,
  long: -3.80567403241881,
  lines: [12],
  name: "Parque Europa",
};

const fuenlabradaCentral = {
  id: "fuenlabradaCentral",
  lat: 40.28265261663439,
  long: -3.799816254313558,
  lines: [12],
  name: "Fuenlabrada Central",
};

const parqueDeLosEstados = {
  id: "parqueDeLosEstados",
  lat: 40.28653199684358,
  long: -3.7870287586446385,
  lines: [12],
  name: "Parque de los Estados",
};

const arroyoCulebro = {
  id: "arroyoCulebro",
  lat: 40.28903413897547,
  long: -3.7595453039475544,
  lines: [12],
  name: "Arroyo Culebro",
};

const conservatorio = {
  id: "conservatorio",
  lat: 40.29358060460064,
  long: -3.747981943166903,
  lines: [12],
  name: "Conservatorio",
};

const alonsoDeMendoza = {
  id: "alonsoDeMendoza",
  lat: 40.30073309139775,
  long: -3.7397167233982254,
  lines: [12],
  name: "Alonso De Mendoza",
};

const getafeCentral = {
  id: "getafeCentral",
  lat: 40.30970293670267,
  long: -3.7367762124897723,
  lines: [12],
  name: "Getafe Central",
};

const juanDeLaCierva = {
  id: "juanDeLaCierva",
  lat: 40.31161190543185,
  long: -3.72616653114579,
  lines: [12],
  name: "Juan de La Cierva",
};

const elCasar = {
  id: "elCasar",
  lat: 40.31844646903861,
  long: -3.711085383789591,
  lines: [12],
  name: "El Casar",
};

const losEspartales = {
  id: "losEspartales",
  lat: 40.32429365180987,
  long: -3.7205029660628655,
  lines: [12],
  name: "Los Espartales",
};

const elBercial = {
  id: "elBercial",
  lat: 40.32923155161688,
  long: -3.728132399751428,
  lines: [12],
  name: "El Bercial",
};

const elCarrascal = {
  id: "elCarrascal",
  lat: 40.33659228986702,
  long: -3.7418812748821435,
  lines: [12],
  name: "El Carrascal",
};

const julianBesteiro = {
  id: "julianBesteiro",
  lat: 40.334521489754074,
  long: -3.753683868454276,
  lines: [12],
  name: "Julian Besteiro",
};

const casaDelReloj = {
  id: "casaDelReloj",
  lat: 40.32702346148158,
  long: -3.758546651012411,
  lines: [12],
  name: "Casa del Reloj",
};

const hospitalSeveroOchoa = {
  id: "hospitalSeveroOchoa",
  lat: 40.32176863152142,
  long: -3.7692656691228903,
  lines: [12],
  name: "Hospital Severo Ochoa",
};

const leganesCentral = {
  id: "leganesCentral",
  lat: 40.32858715066292,
  long: -3.7721185456851845,
  lines: [12],
  name: "Leganés Central",
};

const sanNicasio = {
  id: "sanNicasio",
  lat: 40.336152098776516,
  long: -3.7771242064288706,
  lines: [12],
  name: "San Nicasio",
};

// Edges
pinarDeChamartin.edges = [bambu, manoteras];
bambu.edges = [pinarDeChamartin, chamartin];
chamartin.edges = [plazaDeCastilla, bambu, begona];
plazaDeCastilla.edges = [
  chamartin,
  valdeacederas,
  ventilla,
  duqueDePastrana,
  cuzco,
];
valdeacederas.edges = [plazaDeCastilla, tetuan];
tetuan.edges = [valdeacederas, estrecho];
estrecho.edges = [tetuan, alvarado];
alvarado.edges = [estrecho, cuatroCaminos];
cuatroCaminos.edges = [
  alvarado,
  riosRosas,
  canal,
  nuevosMinisterios,
  guzmanElBueno,
];
riosRosas.edges = [cuatroCaminos, iglesia];
iglesia.edges = [riosRosas, bilbao];
bilbao.edges = [iglesia, tribunal, sanBernardo, alonsoMartinez];
tribunal.edges = [bilbao, granVia, alonsoMartinez, plazaDeEspana];
granVia.edges = [tribunal, sol, chueca, callao];
sol.edges = [granVia, tirsoDeMolina, sevilla, opera, lavapies, callao];
tirsoDeMolina.edges = [sol, antonMartin];
antonMartin.edges = [tirsoDeMolina, estacionDelArte];
estacionDelArte.edges = [antonMartin, atocha];
atocha.edges = [estacionDelArte, menendezPelayo];
menendezPelayo.edges = [atocha, pacifico];
pacifico.edges = [menendezPelayo, puenteDeVallecas, mendezAlvaro, condeDeCasal];
puenteDeVallecas.edges = [pacifico, nuevaNumancia];
nuevaNumancia.edges = [puenteDeVallecas, portazgo];
portazgo.edges = [nuevaNumancia, buenosAires];
buenosAires.edges = [portazgo, altoDeArenal];
altoDeArenal.edges = [buenosAires, miguelHernandez];
miguelHernandez.edges = [altoDeArenal, sierraDeGuadalupe];
sierraDeGuadalupe.edges = [miguelHernandez, villaDeVallecas];
villaDeVallecas.edges = [sierraDeGuadalupe, congosto];
congosto.edges = [villaDeVallecas, laGavia];
laGavia.edges = [congosto, lasSuertes];
lasSuertes.edges = [laGavia, valdecarros];
valdecarros.edges = [lasSuertes];

lasRosas.edges = [avenidaDeGuadalajara];
avenidaDeGuadalajara.edges = [lasRosas, alsacia];
alsacia.edges = [avenidaDeGuadalajara, laAlmudena];
laAlmudena.edges = [alsacia, laElipa];
laElipa.edges = [laAlmudena, ventas];
ventas.edges = [laElipa, manuelBecerra, elCarmen, diegoDeLeon];
manuelBecerra.edges = [ventas, goya, odonnell, diegoDeLeon];
goya.edges = [manuelBecerra, principeDeVergara, velazquez, lista];
principeDeVergara.edges = [goya, retiro, nunezDeBalboa, ibiza];
retiro.edges = [principeDeVergara, bancoDeEspana];
bancoDeEspana.edges = [retiro, sevilla];
sevilla.edges = [bancoDeEspana, sol];
opera.edges = [sol, santoDomingo, callao, laLatina, principePio];
santoDomingo.edges = [opera, plazaDeEspana];
sanBernardo.edges = [plazaDeEspana, quevedo, arguelles, bilbao];
quevedo.edges = [sanBernardo, canal];
canal.edges = [quevedo, cuatroCaminos, alonsoCano, islasFilipinas];

villaverdeAlto.edges = [sanCristobal];
sanCristobal.edges = [villaverdeAlto, villaverdeBajoCruce];
villaverdeBajoCruce.edges = [sanCristobal, ciudadDeLosAngeles];
ciudadDeLosAngeles.edges = [villaverdeBajoCruce, sanFerminOrcasur];
sanFerminOrcasur.edges = [ciudadDeLosAngeles, hospital12deOctubre];
hospital12deOctubre.edges = [sanFerminOrcasur, almendrales];
almendrales.edges = [hospital12deOctubre, legazpi];
legazpi.edges = [almendrales, delicias, usera, arganzuelaPlanetario];
delicias.edges = [legazpi, palosDeLaFrontera];
palosDeLaFrontera.edges = [delicias, embajadores];
embajadores.edges = [palosDeLaFrontera, lavapies, puertaDeToledo, piramides];
lavapies.edges = [embajadores, sol];
callao.edges = [sol, plazaDeEspana, granVia, opera];
plazaDeEspana.edges = [
  santoDomingo,
  sanBernardo,
  callao,
  venturaRodriguez,
  tribunal,
  principePio,
];
venturaRodriguez.edges = [plazaDeEspana, arguelles];
arguelles.edges = [venturaRodriguez, moncloa, sanBernardo, principePio];
moncloa.edges = [arguelles, ciudadUniversitaria];

alonsoMartinez.edges = [
  bilbao,
  colon,
  rubenDario,
  chueca,
  gregorioMaranon,
  tribunal,
];
colon.edges = [alonsoMartinez, serrano];
serrano.edges = [colon, velazquez];
velazquez.edges = [serrano, goya];
lista.edges = [goya, diegoDeLeon];
diegoDeLeon.edges = [
  lista,
  avenidaDeAmerica,
  ventas,
  nunezDeBalboa,
  manuelBecerra,
];
avenidaDeAmerica.edges = [
  diegoDeLeon,
  prosperidad,
  republicaArgentina,
  cartagena,
  gregorioMaranon,
  cruzDelRayo,
  nunezDeBalboa,
];
prosperidad.edges = [avenidaDeAmerica, alfonsoXIII];
alfonsoXIII.edges = [prosperidad, avenidaDeLaPaz];
avenidaDeLaPaz.edges = [alfonsoXIII, arturoSoria];
arturoSoria.edges = [avenidaDeLaPaz, esperanza];
esperanza.edges = [arturoSoria, canillas];
canillas.edges = [esperanza, marDeCristal];
marDeCristal.edges = [canillas, sanLorenzo, pinarDelRey, feriaDeMadrid];
sanLorenzo.edges = [marDeCristal, parqueDeSantaMaria];
parqueDeSantaMaria.edges = [sanLorenzo, hortaleza];
hortaleza.edges = [parqueDeSantaMaria, manoteras];
manoteras.edges = [hortaleza, pinarDeChamartin];

alamedaDeOsuna.edges = [elCapricho];
elCapricho.edges = [alamedaDeOsuna, canillejas];
canillejas.edges = [elCapricho, torreArias];
torreArias.edges = [canillejas, suanzes];
suanzes.edges = [torreArias, ciudadLineal];
ciudadLineal.edges = [suanzes, puebloNuevo];
puebloNuevo.edges = [ciudadLineal, quintana, ascao, barrioDeLaConcepcion];
quintana.edges = [puebloNuevo, elCarmen];
elCarmen.edges = [quintana, ventas];
nunezDeBalboa.edges = [diegoDeLeon, rubenDario];
rubenDario.edges = [nunezDeBalboa, alonsoMartinez];
chueca.edges = [alonsoMartinez, granVia];
laLatina.edges = [opera, puertaDeToledo];
puertaDeToledo.edges = [laLatina, acacias];
acacias.edges = [palosDeLaFrontera, lavapies, puertaDeToledo, piramides];
piramides.edges = [acacias, marquesDeVadillo];
marquesDeVadillo.edges = [piramides, urgel];
urgel.edges = [marquesDeVadillo, oporto];
oporto.edges = [urgel, vistaAlegre, carpetana, opanel];
vistaAlegre.edges = [oporto, carabanchel];
carabanchel.edges = [vistaAlegre, eugeniaDeMontijo];
eugeniaDeMontijo.edges = [carabanchel, aluche];
aluche.edges = [eugeniaDeMontijo, empalme];
empalme.edges = [aluche, campamento];
campamento.edges = [empalme, casaDeCampo];
casaDeCampo.edges = [campamento, batan, coloniaJardin];

laguna.edges = [carpetana, lucero];
carpetana.edges = [laguna, oporto];
opanel.edges = [oporto, plazaEliptica];
plazaEliptica.edges = [opanel, usera, abrantes];
usera.edges = [plazaEliptica, legazpi];
arganzuelaPlanetario.edges = [usera, mendezAlvaro];
mendezAlvaro.edges = [arganzuelaPlanetario, pacifico];
condeDeCasal.edges = [pacifico, sainzDeBaranda];
sainzDeBaranda.edges = [condeDeCasal, odonnell, ibiza, estrella];
odonnell.edges = [sainzDeBaranda, manuelBecerra];
republicaArgentina.edges = [avenidaDeAmerica, nuevosMinisterios];
nuevosMinisterios.edges = [
  republicaArgentina,
  cuatroCaminos,
  colombia,
  santiagoBernabeu,
  gregorioMaranon,
];
guzmanElBueno.edges = [
  cuatroCaminos,
  vicenteAleixandre,
  islasFilipinas,
  francosRodriguez,
];
vicenteAleixandre.edges = [guzmanElBueno, ciudadUniversitaria];
ciudadUniversitaria.edges = [vicenteAleixandre, moncloa];
principePio.edges = [arguelles, puertaDelAngel, plazaDeEspana, lago, opera];
puertaDelAngel.edges = [principePio, altoDeExtremadura];
altoDeExtremadura.edges = [puertaDelAngel, lucero];
lucero.edges = [altoDeExtremadura, laguna];

hospitalDeHenares.edges = [henares];
henares.edges = [hospitalDeHenares, jarama];
jarama.edges = [henares, sanFernando];
sanFernando.edges = [jarama, laRambla];
laRambla.edges = [sanFernando, cosladaCentral];
cosladaCentral.edges = [laRambla, barrioDelPuerto];
barrioDelPuerto.edges = [cosladaCentral, estadioMetropolitano];
estadioMetropolitano.edges = [barrioDelPuerto, lasMusas];
lasMusas.edges = [estadioMetropolitano, sanBlas];
sanBlas.edges = [lasMusas, simancas];
simancas.edges = [sanBlas, garciaNoblejas];
garciaNoblejas.edges = [simancas, ascao];
ascao.edges = [garciaNoblejas, puebloNuevo];
barrioDeLaConcepcion.edges = [puebloNuevo, parqueDeLasAvenidas];
parqueDeLasAvenidas.edges = [barrioDeLaConcepcion, cartagena];
cartagena.edges = [parqueDeLasAvenidas, avenidaDeAmerica];
gregorioMaranon.edges = [
  avenidaDeAmerica,
  alonsoCano,
  nuevosMinisterios,
  alonsoMartinez,
];
alonsoCano.edges = [gregorioMaranon, canal];
islasFilipinas.edges = [canal, guzmanElBueno];
francosRodriguez.edges = [guzmanElBueno, valdezarza];
valdezarza.edges = [francosRodriguez, antonioMachado];
antonioMachado.edges = [valdezarza, penagrande];
penagrande.edges = [antonioMachado, avenidaDeLaIlustracion];
avenidaDeLaIlustracion.edges = [penagrande, lacoma];
lacoma.edges = [avenidaDeLaIlustracion, arroyofresno];
arroyofresno.edges = [lacoma, pitis];
pitis.edges = [arroyofresno];

colombia.edges = [nuevosMinisterios, pinarDelRey, pioXII, conchaEspina];
pinarDelRey.edges = [colombia, marDeCristal];
feriaDeMadrid.edges = [marDeCristal, aeropuertoT1T2T3];
aeropuertoT1T2T3.edges = [feriaDeMadrid, barajas];
barajas.edges = [aeropuertoT1T2T3, aeropuertoT4];
aeropuertoT4.edges = [barajas];
pacoDeLucia.edges = [mirasierra];
mirasierra.edges = [pacoDeLucia, herreraOria];
herreraOria.edges = [mirasierra, barrioDelPilar];
barrioDelPilar.edges = [herreraOria, ventilla];
ventilla.edges = [barrioDelPilar, plazaDeCastilla];
duqueDePastrana.edges = [plazaDeCastilla, pioXII];
pioXII.edges = [duqueDePastrana, colombia];
conchaEspina.edges = [colombia, cruzDelRayo];
cruzDelRayo.edges = [conchaEspina, avenidaDeAmerica];
ibiza.edges = [principeDeVergara, sainzDeBaranda];
estrella.edges = [sainzDeBaranda, vinateros];
vinateros.edges = [estrella, artilleros];
artilleros.edges = [vinateros, pavones];
pavones.edges = [artilleros, valdebernardo];
valdebernardo.edges = [pavones, vicalvaro];
vicalvaro.edges = [valdebernardo, sanCipriano];
sanCipriano.edges = [vicalvaro, puertaDeArganda];
puertaDeArganda.edges = [sanCipriano, rivasUrbanizaciones];
rivasUrbanizaciones.edges = [puertaDeArganda, rivasFutura];
rivasFutura.edges = [rivasUrbanizaciones, rivasVaciamadrid];
rivasVaciamadrid.edges = [rivasFutura, laPoveda];
laPoveda.edges = [rivasVaciamadrid, argandaDelRey];
argandaDelRey.edges = [laPoveda];

hospitalInfantaSofia.edges = [reyesCatolicos];
reyesCatolicos.edges = [hospitalInfantaSofia, baunatal];
baunatal.edges = [reyesCatolicos, manuelDeFalla];
manuelDeFalla.edges = [baunatal, marquesDeLaValdavia];
marquesDeLaValdavia.edges = [manuelDeFalla, laMoraleja];
laMoraleja.edges = [marquesDeLaValdavia, laGranja];
laGranja.edges = [laMoraleja, rondaDeLaComunicacion];
rondaDeLaComunicacion.edges = [laGranja, lasTablas];
lasTablas.edges = [rondaDeLaComunicacion, montecarmelo];
montecarmelo.edges = [lasTablas, tresOlivos];
tresOlivos.edges = [montecarmelo, fuencarral];
fuencarral.edges = [tresOlivos, begona];
begona.edges = [fuencarral, chamartin];
cuzco.edges = [plazaDeCastilla, santiagoBernabeu];
santiagoBernabeu.edges = [cuzco, nuevosMinisterios];
lago.edges = [principePio, batan];
batan.edges = [lago, casaDeCampo];
coloniaJardin.edges = [casaDeCampo, aviacionEspanola];
aviacionEspanola.edges = [coloniaJardin, cuatroVientos];
cuatroVientos.edges = [aviacionEspanola, joaquinVilumbrales];
joaquinVilumbrales.edges = [cuatroVientos, puertaDelSur];
puertaDelSur.edges = [joaquinVilumbrales, parqueDeLisboa, sanNicasio];

// Line 11
abrantes.edges = [plazaEliptica, panBendito];
panBendito.edges = [abrantes, sanFrancisco];
sanFrancisco.edges = [panBendito, carabanchelAlto];
carabanchelAlto.edges = [sanFrancisco, laPeseta];
laPeseta.edges = [carabanchelAlto, laFortuna];
laFortuna.edges = [laPeseta];

// Line 12
parqueDeLisboa.edges = [puertaDelSur, alcorconCentral];
alcorconCentral.edges = [parqueDeLisboa, parqueOeste];
parqueOeste.edges = [alcorconCentral, universidadReyJuanCarlos];
universidadReyJuanCarlos.edges = [parqueOeste, mostolesCentral];
mostolesCentral.edges = [universidadReyJuanCarlos, pradillo];
pradillo.edges = [mostolesCentral, hospitalDeMostoles];
hospitalDeMostoles.edges = [pradillo, manuelaMalasana];
manuelaMalasana.edges = [hospitalDeMostoles, loranca];
loranca.edges = [manuelaMalasana, hospitalDeFuenlabrada];
hospitalDeFuenlabrada.edges = [loranca, parqueEuropa];
parqueEuropa.edges = [hospitalDeFuenlabrada, fuenlabradaCentral];
fuenlabradaCentral.edges = [parqueEuropa, parqueDeLosEstados];
parqueDeLosEstados.edges = [fuenlabradaCentral, arroyoCulebro];
arroyoCulebro.edges = [parqueDeLosEstados, conservatorio];
conservatorio.edges = [arroyoCulebro, alonsoDeMendoza];
alonsoDeMendoza.edges = [conservatorio, getafeCentral];
getafeCentral.edges = [alonsoDeMendoza, juanDeLaCierva];
juanDeLaCierva.edges = [getafeCentral, elCasar];
elCasar.edges = [juanDeLaCierva, losEspartales];
losEspartales.edges = [elCasar, elBercial];
elBercial.edges = [losEspartales, elCarrascal];
elCarrascal.edges = [elBercial, julianBesteiro];
julianBesteiro.edges = [elCarrascal, casaDelReloj];
casaDelReloj.edges = [julianBesteiro, hospitalSeveroOchoa];
hospitalSeveroOchoa.edges = [casaDelReloj, leganesCentral];
leganesCentral.edges = [hospitalSeveroOchoa, sanNicasio];
sanNicasio.edges = [leganesCentral, puertaDelSur];

// Lineas

const linea1 = {
  stops: [
    pinarDeChamartin,
    bambu,
    chamartin,
    plazaDeCastilla,
    valdeacederas,
    tetuan,
    estrecho,
    alvarado,
    cuatroCaminos,
    riosRosas,
    iglesia,
    bilbao,
    tribunal,
    granVia,
    sol,
    tirsoDeMolina,
    antonMartin,
    estacionDelArte,
    atocha,
    menendezPelayo,
    pacifico,
    puenteDeVallecas,
    nuevaNumancia,
    portazgo,
    buenosAires,
    altoDeArenal,
    miguelHernandez,
    sierraDeGuadalupe,
    villaDeVallecas,
    congosto,
    laGavia,
    lasSuertes,
    valdecarros,
  ],
  color: "#67c0dd",
};

const linea2 = {
  stops: [
    lasRosas,
    avenidaDeGuadalajara,
    alsacia,
    laAlmudena,
    laElipa,
    ventas,
    manuelBecerra,
    goya,
    principeDeVergara,
    retiro,
    bancoDeEspana,
    sevilla,
    sol,
    opera,
    santoDomingo,
    plazaDeEspana,
    sanBernardo,
    quevedo,
    canal,
    cuatroCaminos,
  ],
  color: "#e0292f",
};

const linea3 = {
  stops: [
    villaverdeAlto,
    sanCristobal,
    villaverdeBajoCruce,
    ciudadDeLosAngeles,
    sanFerminOrcasur,
    hospital12deOctubre,
    almendrales,
    legazpi,
    delicias,
    palosDeLaFrontera,
    embajadores,
    lavapies,
    sol,
    callao,
    plazaDeEspana,
    venturaRodriguez,
    arguelles,
    moncloa,
  ],
  color: "#fdd700",
};

const linea4 = {
  stops: [
    arguelles,
    sanBernardo,
    bilbao,
    alonsoMartinez,
    colon,
    serrano,
    velazquez,
    goya,
    lista,
    diegoDeLeon,
    avenidaDeAmerica,
    prosperidad,
    alfonsoXIII,
    avenidaDeLaPaz,
    arturoSoria,
    esperanza,
    canillas,
    marDeCristal,
    sanLorenzo,
    parqueDeSantaMaria,
    hortaleza,
    manoteras,
    pinarDeChamartin,
  ],
  color: "#cf7700",
};

const linea5 = {
  stops: [
    alamedaDeOsuna,
    elCapricho,
    canillejas,
    torreArias,
    suanzes,
    ciudadLineal,
    puebloNuevo,
    quintana,
    elCarmen,
    ventas,
    diegoDeLeon,
    nunezDeBalboa,
    rubenDario,
    alonsoMartinez,
    chueca,
    granVia,
    callao,
    opera,
    laLatina,
    puertaDeToledo,
    acacias,
    piramides,
    marquesDeVadillo,
    urgel,
    oporto,
    vistaAlegre,
    carabanchel,
    eugeniaDeMontijo,
    aluche,
    empalme,
    campamento,
    casaDeCampo,
  ],
  color: "#98be13",
};

const linea6 = {
  stops: [
    laguna,
    carpetana,
    oporto,
    opanel,
    plazaEliptica,
    usera,
    legazpi,
    arganzuelaPlanetario,
    mendezAlvaro,
    pacifico,
    condeDeCasal,
    sainzDeBaranda,
    odonnell,
    manuelBecerra,
    diegoDeLeon,
    avenidaDeAmerica,
    republicaArgentina,
    nuevosMinisterios,
    cuatroCaminos,
    guzmanElBueno,
    vicenteAleixandre,
    ciudadUniversitaria,
    moncloa,
    arguelles,
    principePio,
    puertaDelAngel,
    altoDeExtremadura,
    lucero,
  ],
  color: "#c0bab0",
};

const linea7 = {
  stops: [
    hospitalDeHenares,
    henares,
    jarama,
    sanFernando,
    laRambla,
    cosladaCentral,
    barrioDelPuerto,
    estadioMetropolitano,
    lasMusas,
    sanBlas,
    simancas,
    garciaNoblejas,
    ascao,
    puebloNuevo,
    barrioDeLaConcepcion,
    parqueDeLasAvenidas,
    cartagena,
    avenidaDeAmerica,
    gregorioMaranon,
    alonsoCano,
    canal,
    islasFilipinas,
    guzmanElBueno,
    francosRodriguez,
    valdezarza,
    antonioMachado,
    penagrande,
    avenidaDeLaIlustracion,
    lacoma,
    arroyofresno,
    pitis,
  ],
  color: "#f19c0c",
};

const linea8 = {
  stops: [
    nuevosMinisterios,
    colombia,
    pinarDelRey,
    marDeCristal,
    feriaDeMadrid,
    aeropuertoT1T2T3,
    barajas,
    aeropuertoT4,
  ],
  color: "#eba6c9",
};

const linea9 = {
  stops: [
    pacoDeLucia,
    mirasierra,
    herreraOria,
    barrioDelPilar,
    ventilla,
    plazaDeCastilla,
    duqueDePastrana,
    pioXII,
    colombia,
    conchaEspina,
    cruzDelRayo,
    avenidaDeAmerica,
    nunezDeBalboa,
    principeDeVergara,
    ibiza,
    sainzDeBaranda,
    estrella,
    vinateros,
    artilleros,
    pavones,
    valdebernardo,
    vicalvaro,
    sanCipriano,
    puertaDeArganda,
    rivasUrbanizaciones,
    rivasFutura,
    rivasVaciamadrid,
    laPoveda,
    argandaDelRey,
  ],
  color: "#9e2f8a",
};

const linea10 = {
  stops: [
    hospitalInfantaSofia,
    reyesCatolicos,
    baunatal,
    manuelDeFalla,
    marquesDeLaValdavia,
    laMoraleja,
    laGranja,
    rondaDeLaComunicacion,
    lasTablas,
    montecarmelo,
    tresOlivos,
    fuencarral,
    begona,
    chamartin,
    plazaDeCastilla,
    cuzco,
    santiagoBernabeu,
    nuevosMinisterios,
    gregorioMaranon,
    alonsoMartinez,
    tribunal,
    plazaDeEspana,
    principePio,
    lago,
    batan,
    casaDeCampo,
    coloniaJardin,
    aviacionEspanola,
    cuatroVientos,
    joaquinVilumbrales,
    puertaDelSur,
  ],
  color: "#005fa9",
};

const linea11 = {
  stops: [
    plazaEliptica,
    abrantes,
    panBendito,
    sanFrancisco,
    carabanchelAlto,
    laPeseta,
    laFortuna,
  ],
  color: "#009739",
};

const linea12 = {
  stops: [
    puertaDelSur,
    parqueDeLisboa,
    alcorconCentral,
    parqueOeste,
    universidadReyJuanCarlos,
    mostolesCentral,
    pradillo,
    hospitalDeMostoles,
    manuelaMalasana,
    loranca,
    hospitalDeFuenlabrada,
    parqueEuropa,
    fuenlabradaCentral,
    parqueDeLosEstados,
    arroyoCulebro,
    conservatorio,
    alonsoDeMendoza,
    getafeCentral,
    juanDeLaCierva,
    elCasar,
    losEspartales,
    elBercial,
    elCarrascal,
    julianBesteiro,
    casaDelReloj,
    hospitalSeveroOchoa,
    leganesCentral,
    sanNicasio,
  ],
  color: "#b9ab1f",
};

const linea13 = {
  stops: [opera, principePio],
  color: "#333b62",
};

const lineas = [
  linea1,
  linea2,
  linea3,
  linea4,
  linea5,
  linea6,
  linea7,
  linea8,
  linea9,
  linea10,
  linea11,
  linea12,
  linea13,
];

var connectors = [];

const resultado = (coordenada1, coordenada2) => {
  var temporal = 9999999999999999999999999999999999999999999999999999999999999999;
  var nombre = null;
  var stopMasChico = null;
  for (linea of lineas) {
    for (stop of linea.stops) {
      if (
        ((stop.lat - coordenada1) ** 2 + (stop.long - coordenada2) ** 2) **
          (1 / 2) <
        temporal
      ) {
        temporal =
          ((stop.lat - coordenada1) ** 2 + (stop.long - coordenada2) ** 2) **
          (1 / 2);
        nombre = stop.name;
        stopMasChico = stop;
      }
    }
  }
  return [stopMasChico, nombre];
};

const clearMap = () => {
  for (linea of lineas) {
    for (stop of linea.stops) {
      document.getElementById(stop.id).style.display = "none";
    }
  }
  for (connector of connectors) {
    document.getElementById(connector).style.display = "none";
  }
};

const loadMap = () => {
  var offsetTop = document
    .getElementById("miniWindow")
    .getBoundingClientRect().top;
  var offsetLeft = document
    .getElementById("miniWindow")
    .getBoundingClientRect().left;
  var visited = new Set();
  for (linea of lineas) {
    var lineaColor = linea.color;
    var prevX = null;
    var prevY = null;
    var prevID = null;
    for (stop of linea.stops) {
      var xCoord = (3.872801530092762 + stop.long) * 140.106355446;
      var yCoord = (40.560110329013575 - stop.lat) * 259.499003948;
      if (!visited.has(stop.id)) {
        document.getElementById(
          "miniWindow"
        ).innerHTML += `<div id="${stop.id}" class="stop" 
                      style="top: calc(${offsetTop}px + ${yCoord}vh + 2vh); left: calc(${offsetLeft}px + ${xCoord}vh + 2vh)">
        <div class="stopPoint" style="border: 1px solid black; background: white"></div>
        <div class="stopName" style="color: ${lineaColor}">${stop.name}</div> 
        </div>`;
        visited.add(stop.id);
      }
      xCoord = document.getElementById(stop.id).getBoundingClientRect().left;
      yCoord = document.getElementById(stop.id).getBoundingClientRect().top;
      if (prevID && !connectors.includes(`${prevID},${stop.id}`)) {
        document.getElementById(
          "miniWindow"
        ).innerHTML += `<svg id="${prevID},${stop.id}"><polyline points="${xCoord},${yCoord} ${prevX},${prevY}" style="stroke: ${lineaColor}"></polyline></svg>`;
        connectors.push(`${prevID},${stop.id}`);
      }
      if (stop == sanNicasio) {
        prevID = "puertaDelSur";
        prevX = document.getElementById(prevID).getBoundingClientRect().left;
        prevY = document.getElementById(prevID).getBoundingClientRect().top;
        document.getElementById(
          "miniWindow"
        ).innerHTML += `<svg id="${prevID},${stop.id}"><polyline points="${xCoord},${yCoord} ${prevX},${prevY}" style="stroke: ${lineaColor}"></polyline></svg>`;
        connectors.push(`${prevID},${stop.id}`);
      }
      prevID = stop.id;
      prevX = xCoord;
      prevY = yCoord;
    }
  }
};

const updateMap = (route) => {
  clearMap();
  var prevID = null;
  for (stop of route) {
    document.getElementById(stop.id).style.display = "flex";
    if (prevID && document.getElementById(`${prevID},${stop.id}`)) {
      document.getElementById(`${prevID},${stop.id}`).style.display = "flex";
    } else if (prevID && document.getElementById(`${stop.id},${prevID}`)) {
      document.getElementById(`${stop.id},${prevID}`).style.display = "flex";
    }
    prevID = stop.id;
  }
};

const findShortestRoute = (startStop, endStop) => {
  const queue = [[startStop]];
  const visited = new Set();

  while (queue.length > 0) {
    const path = queue.shift();
    const stop = path[path.length - 1];

    if (stop == endStop) {
      return path;
    }

    if (!visited.has(stop)) {
      visited.add(stop);
      const neighbors = stop.edges;

      for (const neighbor of neighbors) {
        const newPath = path.concat([neighbor]);
        queue.push(newPath);
      }
    }
  }
};

const processShortestPath = (path) => {
  updateMap(path);
  document.getElementById("divInstrucciones").innerHTML = "";
  for (stop of path) {
    document.getElementById(
      "divInstrucciones"
    ).innerHTML += `<li>${stop.name}</li>`;
  }
};

const createRoute = () => {
  const fromCoords = document
    .getElementById("input1")
    .value.split(",")
    .map(Number);
  const toCoords = document
    .getElementById("input2")
    .value.split(",")
    .map(Number);
  const startStop = resultado(fromCoords[0], fromCoords[1])[0];
  const endStop = resultado(toCoords[0], toCoords[1])[0];

  processShortestPath(findShortestRoute(startStop, endStop));
};

const useGPS = () => {
  if (window.innerWidth < window.innerHeight) {
    navigator.geolocation.getCurrentPosition(
      function (position) {
        document.getElementById(
          "input1"
        ).value = `${position.coords.latitude},${position.coords.longitude}`;
      },
      function (error) {
        console.error("Error getting location: " + error.message);
      }
    );
  }
};
